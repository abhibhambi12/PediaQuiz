// --- CORRECTED FILE: workspaces/frontend/src/pages/CustomTestBuilder.tsx ---
...
const CustomTestBuilder: React.FC = () => {
    ...
    const handleStartTest = async () => {
        ...
        if (chapterIds.length === 0 || !user) {
            addToast("Please select at least one chapter.", "warning");
        ...
        try {
            const chapterContents = await Promise.all(chapterIds.map(id => getChapterContent(id)));
            const allAvailableMcqs = chapterContents.flatMap(content => content.mcqs);
    ...
    const isStartButtonDisabled = selectedChapters.size === 0 || totalQuestions <= 0 || selectedMcqCount < totalQuestions || isCreatingTest;
