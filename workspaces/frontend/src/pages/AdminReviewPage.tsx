// FILE: workspaces/frontend/src/pages/AdminReviewPage.tsx
...
import type { UserUpload, UploadStatus, Topic, Chapter, AssignmentSuggestion, MCQ, Flashcard } from '@pediaquiz/types';
...
const UploadCard: React.FC<{ upload: UserUpload, allTopics: Topic[] }> = ({ upload, allTopics }) => {
    ...
    // Smart Marrow Text Pipeline specific state
    const [smartMarrowNumToGenerate, setSmartMarrowNumToGenerate] = useState(upload.suggestedNewMcqCount || 10);

    // General Pipeline specific state
    const [generalNumToGenerate, setGeneralNumToGenerate] = useState(upload.estimatedMcqCount || 10); // Changed from generalMcqCount
    ...
    React.useEffect(() => {
        ...
        setSmartMarrowNumToGenerate(upload.suggestedNewMcqCount || 10);
    }, [upload]);


    const isMarrowUpload = upload.fileName.startsWith("MARROW_");

    const filteredTopicsForSelection = useMemo(() => allTopics.filter(t => isMarrowUpload ? t.source === 'Marrow' : t.source === 'General'), [allTopics, isMarrowUpload]);
    const availableChaptersForMarrowTopic = useMemo(() => allTopics.find(t => t.id === marrowSelectedTopic)?.chapters || [], [allTopics, marrowSelectedTopic]);
    ...
    // Callable Function References (These are the direct references, not mutation objects)
    ...
    const startAutomatedBatchGenerationCallable = httpsCallable<{ uploadId: string }, { success: boolean }>(functions, 'startAutomatedBatchGeneration'); // This is the callable function
    ...
    // Mutations (These are the objects from useMutation, which have .mutate and .isPending)
    const extractMarrowMutation = useMutation<HttpsCallableResult<{ mcqCount: number; explanationCount: number; }>, Error, string>({ mutationFn: (id: string) => extractMarrowContentCallable({ uploadId: id }), onSuccess: () => {playSound('notification'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const generateAndAnalyzeMarrowMutation = useMutation<HttpsCallableResult<{ success: boolean; message?: string; }>, Error, { uploadId: string; count: number; }>({ mutationFn: (vars) => generateAndAnalyzeMarrowContentCallable(vars), onSuccess: () => {playSound('notification'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const approveMarrowMutation = useMutation<HttpsCallableResult<{ success: boolean; message?: string; }>, Error, { uploadId: string, topicId: string, topicName: string, chapterId: string, chapterName: string, keyTopics: string[] }>({ mutationFn: (vars) => approveMarrowContentCallable(vars), onSuccess: () => {playSound('notification'); addToast("Marrow content approved!", 'success'); queryClient.invalidateQueries({ queryKey: ['pendingUploads', 'topics'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const smartMarrowGenerateMutation = useMutation<HttpsCallableResult<{ success: boolean; }>, Error, { uploadId: string; count: number; }>({ mutationFn: (vars) => generateAndStageMarrowMcqsCallable(vars), onSuccess: () => {playSound('notification'); addToast("Marrow content generated and staged!", 'success'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const suggestClassificationMutation = useMutation<HttpsCallableResult<{ success: boolean; suggestedTopic?: string | undefined; suggestedChapter?: string | undefined; }>, Error, string>({ mutationFn: (id: string) => suggestClassificationCallable({ uploadId: id }), onSuccess: () => {playSound('notification'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const generateGeneralContentMutation = useMutation<HttpsCallableResult<{ success: boolean; }>, Error, { uploadId: string; count: number; }>({ mutationFn: (vars) => generateGeneralContentCallable(vars), onSuccess: () => {playSound('notification'); addToast("General content generation complete!", 'success'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] }); }, onError: (error) => {playSound('incorrect'); addToast(`General generation failed: ${error.message}`, 'danger');} });
    const prepareBatchGenerationMutation = useMutation<HttpsCallableResult<{ success: boolean; totalBatches: number; }>, Error, { uploadId: string, totalMcqCount: number, totalFlashcardCount: number, batchSize: number, approvedTopic: string, approvedChapter: string }>({ mutationFn: (vars) => prepareBatchGenerationCallable(vars), onSuccess: () => {playSound('notification'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const startAutomatedBatchGenerationMutation = useMutation<HttpsCallableResult<{ success: boolean; }>, Error, string>({ mutationFn: (id: string) => startAutomatedBatchGenerationCallable({ uploadId: id }), onSuccess: () => {playSound('notification'); queryClient.invalidateQueries({ queryKey: ['pendingUploads'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    const approveContentMutation = useMutation<HttpsCallableResult<{ success: boolean; message?: string; }>, Error, { uploadId: string, assignments: AssignmentSuggestion[] }>({ mutationFn: (vars) => approveContentCallable(vars), onSuccess: () => {playSound('notification'); addToast("Content approved successfully!", 'success'); queryClient.invalidateQueries({ queryKey: ['pendingUploads', 'topics'] });}, onError: (e: Error) => {playSound('incorrect'); addToast(e.message, 'danger');} });
    ...
    const handleApproveMarrowContent = () => {
        ...
        // Corrected finalChapterName for CREATE_NEW case
    ...
    const renderActions = () => {
        ...
        const isLoadingMutation = extractMarrowMutation.isPending || generateAndAnalyzeMarrowMutation.isPending || 
                                ...
                                generateGeneralContentMutation.isPending; // Include new mutation
        ...
        if (isMarrowUpload) {
            switch (upload.status) {
                ...
                case 'pending_marrow_generation_approval': // New Stage 2 for Smart Marrow (after text paste or OCR of MARROW_TEXT_ files)
                    ...
                    return (
                        <div className="space-y-3">
                            ...
                            <input type="number" value={smartMarrowNumToGenerate} onChange={e => setSmartMarrowNumToGenerate(parseInt(e.target.value, 10))} placeholder="Enter count" min="0" className="input-field w-full" />
                            <button onClick={() => {playSound('buttonClick'); smartMarrowGenerateMutation.mutate({ uploadId: upload.id, count: smartMarrowNumToGenerate })}} disabled={smartMarrowGenerateMutation.isPending || smartMarrowNumToGenerate < 0} className="btn-primary w-full">Generate New Marrow Content</button>
                            {(smExtractedMcqs.length > 0 && smartMarrowNumToGenerate === 0) && (
                ...
                case 'pending_assignment': // Stage 3 for Marrow (after generation)
                    ...
                    return (
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2"> <label className="block text-sm font-medium mb-1">Key Clinical Topics (Tags)</label> <div className="flex flex-wrap gap-2 p-2 border rounded-md border-neutral-300 dark:border-neutral-600 bg-neutral-50 dark:bg-neutral-700"> {marrowKeyTopics.map(tag => (<span key={tag} className="flex items-center px-2 py-1 rounded-full bg-primary-100 text-primary-700 dark:bg-primary-900/50 dark:text-primary-300 text-sm">{tag}<button onClick={() => handleRemoveMarrowKeyTopic(tag)} className="ml-1 text-primary-500 hover:text-primary-700">×</button></span>))} <input type="text" onKeyDown={handleAddMarrowKeyTopic} placeholder="Add new tag (Enter)" className="flex-grow min-w-[100px] bg-transparent outline-none" /> </div> <p className="text-xs text-neutral-500">AI Suggestions: {upload.suggestedKeyTopics?.join(', ') || 'None'}</p> </div>
                ...
                case 'pending_final_review':
                    ...
                    // Key Concepts Flashcards: Display overall generated content for review
                    ...
                    return (
                        <div className="space-y-4">
                            ...
                            <button onClick={handleFinalApproveAssignments} disabled={approveContentMutation.isPending} className="btn-success w-full mt-4">Final Approve & Save</button>
                        </div>
                    );
                default:
